...
const AreaCalculatorPage = () => {
  ...
  return (
    <>
      <Helmet>
        <title>Kalkulator Luas - Kalkulator Konstruksi</title>
        <meta name="description" content="Hitung biaya pekerjaan konstruksi berdasarkan luas dengan berbagai bentuk geometri" />
      </Helmet>
              ...
              <div className="flex items-center space-x-3 sm:space-x-4 flex-1 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-white truncate flex items-center">
                    <Square className="w-6 h-6 sm:w-8 sm:h-8 mr-2 sm:mr-3" />
                    Kalkulator Luas - {jobType?.name || 'Kalkulator Luas'}
                  </h1>
                  <p className="text-primary-100 mt-1 text-sm sm:text-base lg:text-lg truncate">
                    {jobType ? `${jobType.description || jobType.name} - ${jobType.unit}` : 'Hitung biaya berdasarkan luas dengan berbagai bentuk geometri'}
                  </p>
                </div>
              </div>
